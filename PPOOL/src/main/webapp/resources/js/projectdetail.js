$(document).ready(function (){
	...
	function commentReplyRegister(target){
		$(target).click(function(){
			...
			var result = confirm('댓글을 등록하시겠습니까?');
			if(result) {
				var form = $('#commentform'+no).serialize();
				
				$.ajax({
					url : "/ppool/commentreply.action",
					async : false,
					method : "POST",
					data : form,
					success : function(result) {
						if (result.indexOf("<success/>") == -1) {
							...
							alert('입력 성공!');
							$('#commentcontent'+no).val("");
							var td = $(result.substr(result.indexOf("<td")));
							$( "#tr"+no ).append(td);
							$('.nocomment').css("display", "none");
							...
							addEditListener(td.find(".ed_bt"));
							okListener(td.find(".ok_bt"));
							cancelListener(td.find(".ca_bt"));
							deleteListener(td.find(".de_bt"));
							commentReplyListener(td.find(".re_bt"));
							commentReplyRegister(td.find(".rr_bt"));
						}
					},
					error : function(xhr, status, error) {
						alert('입력이 에러');
